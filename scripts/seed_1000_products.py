...
from models import (
    ...
    RiderEarning, Review, WishlistItem
)
...
async def seed_products():
    ...
    async with SessionLocal() as db:
        try:
            print("Starting Database Product Reset and 1000 Seeding...")
            ...
            await db.commit()
            print("Cleared Products and existing relations.")
            ...
            # User wants "main category", meaning parent_id is None
            cat_res = await db.execute(select(Category).where(Category.parent_id.is_(None)))
            main_categories = cat_res.scalars().all()
            
            if not main_categories:
                # fallback to any categories if no main categories exist
                cat_res = await db.execute(select(Category))
                main_categories = cat_res.scalars().all()
                print("No categories with parent_id None found. Using all categories.")

            if not dealers or not tax_rules or not main_categories:
                msg = f"❌ Missing prerequisite data (Dealers: {len(dealers)}, TaxRules: {len(tax_rules)}, Categories: {len(main_categories)})."
                print(msg)
                with open("output.txt", "a") as f:
                    f.write(msg + "\n")
                return

            print(f"   Using {len(dealers)} dealers, {len(tax_rules)} tax rules, and {len(main_categories)} main categories.")
            
            # Images mapping by category keyword to real unsplash imagery
            images_map = {
                "electronics": [
            ...
            for i in range(1, 1001):
                ...
                elif "home" in cat_name_lower or "furniture" in cat_name_lower:
                    image_set = images_map["home"]
                elif "beauty" in cat_name_lower or "health" in cat_name_lower or "makeup" in cat_name_lower:
                    image_set = images_map["beauty"]
                
                product_images = [random.choice(image_set)]
                ...
                product_sizes = []
                if "men" in cat_name_lower or "women" in cat_name_lower or "kids" in cat_name_lower or "wear" in cat_name_lower or "cloth" in cat_name_lower:
                    product_sizes = ["S", "M", "L", "XL", "XXL"]
                elif "shoe" in cat_name_lower or "footwear" in cat_name_lower:
                    product_sizes = ["6", "7", "8", "9", "10", "11"]

                p = Product(
                    name=f"Premium {category.name} Item {i}",
                    ...
                    brand=random.choice(["PremiumBrand", "NeoStyle", "UrbanCo", "EcoLife", "ProFit"]),
                    color=random.choice(["Black", "White", "Blue", "Red", "Green", "Silver"]),
                    sizes=product_sizes,
                    hsn_code=f"HSN-{8000 + i}",
                ...
                if i % 100 == 0:
                    print(f"   Seeding products... {i}/1000")

            await db.commit()
            print("SUCCESS! reset and seeded 1000 products correctly.")
            with open("output.txt", "a") as f:
                f.write("SUCCESS! reset and seeded 1000 products correctly.\n")
